...
class ItemRequestRepositoryTest {
    ...
    void findAllByRequestorNotLikeOrderByCreatedAscTest() {
        User user = userRepository.save(User.builder().id(1L).name("name").email("email@email.com").build());
        itemRequestRepository.save(ItemRequest.builder().description("description").requestor(user)
                .created(LocalDateTime.now()).build());
        assertThat(itemRequestRepository.findAllByRequestorIdNotOrderByCreatedAsc(1L, Pageable.ofSize(10))
                .stream().count(), equalTo(0L));
        User user2 = userRepository.save(User.builder().id(2L).name("name2").email("email2@email.com").build());
        assertThat(itemRequestRepository.findAllByRequestorIdNotOrderByCreatedAsc(2L, Pageable.ofSize(10))
